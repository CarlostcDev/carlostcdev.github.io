...
let languageState = {code: DEFAULT_LANGUAGE, translations: {}, roles: []};
...
export async function applyLanguage(language) {
    ...
    if (!translations && targetCode !== DEFAULT_LANGUAGE) translations = await loadTranslations(DEFAULT_LANGUAGE);
    if (requestId !== languageRequestId || !translations) return;
    const fallback = targetCode === DEFAULT_LANGUAGE ? translations : await loadTranslations(DEFAULT_LANGUAGE) ?? translations;
    if (requestId !== languageRequestId) return;
    const roles = getValue(translations, "portfolio.home.roles-list") ?? getValue(fallback, "portfolio.home.roles-list") ?? [];
...
function resolveLanguage(language) {
    ...
    for (const item of config.languages) {
        ...
        if (itemCode === normalizedLanguage || itemCode === baseLanguage ||
            aliases.includes(normalizedLanguage) || aliases.includes(baseLanguage)) {
            return item.code;
        }
    }
    return null;
...
function translateDocument(translations, fallback) {
    document.querySelectorAll("[data-i18n]").forEach(element => {
        ...
        if (attribute) element.setAttribute(attribute, String(value));
...
export function bindLanguageSelector() {
    ...
    const setOpen = open => {
        ...
        allOptions.forEach(options => {options.hidden = !open;});
    };

    document.addEventListener("click", event => {
        const toggleButton = event.target.closest("#langSelected, .lang-toggle");
        ...
        if (!event.target.closest(".lang-dropdown")) setOpen(false);
    });
