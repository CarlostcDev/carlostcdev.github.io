...
const hasColorTheme = config.themes.some(theme =>
...
try {
    ...
    theme = storedTheme === "dark-theme" || storedTheme === "light-theme"
        ? storedTheme
        : systemTheme;
} catch {
    theme = systemTheme;
...
function changeTheme(newTheme) {
    ...
    const colorTheme = config.themes.find(colorTheme =>
        root.classList.contains(colorTheme)
    );

    if (favicon && colorTheme) {
        favicon.setAttribute(
            "href",
            `/sources/svgs/favicon/${theme}/favicon.${theme}.${colorTheme}.svg`
        );
    }
}
